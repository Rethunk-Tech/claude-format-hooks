...
func parseCursorHooks(path string) (before []byte, top, hooks *orderedMap, entries []json.RawMessage, err error) {
	...
	if raw, ok := top.Get("hooks"); ok {
		if err := json.Unmarshal(raw, hooks); err != nil {
			return nil, nil, nil, nil, fmt.Errorf("parse %s: hooks: %w", path, err)
		}
	}
	if raw, ok := hooks.Get(cursorEvent); ok {
		if err := json.Unmarshal(raw, &entries); err != nil {
			return nil, nil, nil, nil, fmt.Errorf("parse %s: hooks.%s: %w", path, cursorEvent, err)
		}
	}
...
func WireCursor(path, binPath string) (before, after []byte, err error) {
	before, top, hooks, entries, err := parseCursorHooks(path)
	if err != nil {
		return nil, nil, err
	}
