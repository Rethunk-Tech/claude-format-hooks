...
formats/lints a file right after Write/Edit/MultiEdit/NotebookEdit writes it, with no
...
- **GraphQL (`.graphql`/`.gql`)** — Biome when an upward `biome.json`/
  `biome.jsonc` exists and a `biome`/`bunx` launcher is available;
  otherwise prettier. Same router pattern as `.json` (Biome only with
  project config); `disabledFormatters: ["biome"]` leaves prettier in
  place. `graphqlRouter.Name()` stays `"prettier"`.
...
- **Terraform/HCL** — `terraform fmt`, or `tofu fmt` when `terraform` is
  missing; no Go equivalent invokable in-process without vendoring
  HashiCorp's own `hclwrite`, and (like rustfmt) there's no competing tool
  to weigh. `disabledFormatters: ["terraform"]` covers both binaries.
...
| [`internal/hookio/`](internal/hookio/) | Decodes stdin into a file path: Claude `PostToolUse` fields first, then Cursor `afterFileEdit` top-level `file_path` |
...
| [`internal/formatters/`](internal/formatters/) | One `Formatter` implementation per file type (native: `json.go`, `shell.go`, `golang.go`; routers: `json.go` `jsonRouter`, `graphql.go`; external: `biome.go`, `bunxtool.go`, `sqlfluff.go`, `python.go`, `rust.go`, `terraform.go`, `proto.go`); `exec.go` holds the shared subprocess-run + diagnostic-truncation helper; `binpath.go` holds the shared, cached `lookPath` every external formatter uses instead of calling `exec.LookPath` directly; `writefile.go` holds the shared mode-preserving write every native formatter uses instead of its own stat-then-write; `markdownconfig.go` + `markdownlint-defaults.jsonc` supply markdownlint-cli2's missing user-level config layer |
| [`internal/installer/`](internal/installer/) | Wires/unwires format-dispatch's Claude `PostToolUse` hook in `~/.claude/settings.json` and Cursor `afterFileEdit` hook in `~/.cursor/hooks.json` (`format-dispatch --install`/`--uninstall`); Cursor uses a distinct writer (`cursor.go`) so Claude `Wire` never mutates `hooks.json`. `CURSOR_HOOKS_FILE` overrides the Cursor path. `upgrade.go` downloads and verifies the latest platform release binary (`format-dispatch --upgrade`), supports `--upgrade --dry-run`, and atomically replaces the installed binary without rewriting settings; `tools.go` globally provisions the bunx-dispatched formatters at install time (and pins transitive deps carrying an unpatched advisory) so no registry fetch happens inside the per-file budget; `orderedmap.go` preserves the file's existing key order across the rewrite and a `.bak` backup is written before any real change; both settings/hooks and binary writes go through `writeAtomic` (unique same-dir temp + rename) so a kill mid-write can never truncate the operator's live files |
