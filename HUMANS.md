...
only child under `hooks`, the `hooks` object is omitted too. A pre-existing
empty `"afterFileEdit": []` is treated as no entries and receives the same
cleanup. Use `CURSOR_HOOKS_FILE` when the Cursor hooks file is elsewhere.
